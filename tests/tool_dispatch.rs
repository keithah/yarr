...
async fn snippet_list_is_a_routed_action() {
    // Built-ins are shipped with Code Mode and remain usable even when persistence
    // is disabled because this loopback state has no data directory.
    let state = loopback_state();
    let listed =
        execute_tool_without_peer_for_test(&state, "sonarr", json!({ "action": "snippet_list" }))
            .await
            .expect("snippet_list without a data dir should expose built-ins");
    assert_eq!(listed["snippets"][0]["name"], "fleet_activity");
}
